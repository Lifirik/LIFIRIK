...
void b2ContactSolver_ctor(b2ContactSolver *solver, b2Contact** contacts, int32 contactCount, b2StackAllocator* allocator)
{
	...
	for (int32 i = 0; i < contactCount; ++i)
	{
		...
		for (int32 j = 0; j < manifoldCount; ++j)
		{
			...
			// LIFIRIK addition (belts): a belt surface asks the pair for a
			// nonzero relative tangent velocity instead of zero. The SIGN
			// (which side is "2") is calibrated by probe against the running
			// game, not derived — see sim.js's belt boundary.
			c->tangentSpeed = contact->m_shape2->m_tangentSpeed - contact->m_shape1->m_tangentSpeed;
...
void b2ContactSolver_SolveVelocityConstraints(b2ContactSolver *solver)
{
	for (int32 i = 0; i < solver->m_constraintCount; ++i)
	{
		...
		// Solver tangent constraints
		{
			...
			// LIFIRIK addition (belts): drive the pair toward the surface's
			// own tangent speed rather than zero. c->tangentSpeed is 0 on
			// every non-belt contact, making this the original line.
			float64 vt = b2Dot(dv, tangent) - c->tangentSpeed;
