...
struct b2ShapeDef
{
	...
	float64 restitution;
	float64 density;
	// LIFIRIK addition: conveyor surface speed, units/s along the surface
	// tangent. 0 on everything except belt terrain, and 0 changes nothing —
	// see b2ContactSolver's tangent target. (FC has no such concept; this is
	// the one piece of LIFIRIK that has to live inside the solver.)
	float64 tangentSpeed;
};
...
struct b2Shape
{
	...
	float64 m_tangentSpeed;	// LIFIRIK addition — see b2ShapeDef
