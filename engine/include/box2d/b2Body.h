...
struct b2Body
{
	...
	float64 m_rotation;
	b2Mat22 m_R;
	// The angle m_R was BUILT from — see b2Body_SyncR below.
	float64 m_R_angle;
...
/* m_R is a pure function of m_rotation, and the solvers ask for it again and
 * again with the rotation unchanged (a contact point whose position impulse
 * clamped to zero moves nothing, and every extra contact point on a body
 * repeats its neighbour's angle). Recomputing it costs a sin and a cos —
 * about a third of this engine's whole step. Compared BITWISE, so a cached
 * answer is the answer the trig would have produced, down to the sign of a
 * zero: the skip is not an approximation. */
static void b2Body_SyncR(b2Body *body)
{
	union { float64 d; uint64_t u; } have, want;

	have.d = body->m_R_angle;
	want.d = body->m_rotation;
	if (have.u == want.u)
		return;
	b2Mat22_SetAngle(&body->m_R, body->m_rotation);
	body->m_R_angle = body->m_rotation;
}
